...
// Decode-weighted energy: base + perKTokOut*(completion/1000) + perKTokIn*(prompt/1000)
const energyWh = (tier, promptTokens, completionTokens) => {
...
test("energy is decode-weighted and carbon = energy/1000 * intensity", () => {
  const gPerKwh = 450;
  const m = compute({ model: "gpt-4o-mini", tier: "small", promptTokens: 500, completionTokens: 1500, gPerKwh, cached: false });
  const expectedWh = energyWh("small", 500, 1500);
  approx(m.actual.energyWh, expectedWh);
  approx(m.actual.carbonG, (expectedWh / 1000) * gPerKwh);
  assert.equal(m.totalTokens, 2000);
});

test("energy is materially more sensitive to completion tokens than prompt tokens", () => {
  const base = compute({ model: "gpt-4o-mini", tier: "small", promptTokens: 100, completionTokens: 100, gPerKwh: 450, cached: false }).actual.energyWh;
  const morePrompt = compute({ model: "gpt-4o-mini", tier: "small", promptTokens: 1100, completionTokens: 100, gPerKwh: 450, cached: false }).actual.energyWh;
  const moreCompletion = compute({ model: "gpt-4o-mini", tier: "small", promptTokens: 100, completionTokens: 1100, gPerKwh: 450, cached: false }).actual.energyWh;
  const dPrompt = morePrompt - base, dCompletion = moreCompletion - base;
  assert.ok(dCompletion > dPrompt * 5, `+1000 completion (${dCompletion.toFixed(4)}) should dwarf +1000 prompt (${dPrompt.toFixed(4)})`);
});
...
test("baseline always uses the large tier / large model", () => {
  ...
  approx(m.baseline.energyWh, energyWh("large", 1000, 1000));
  approx(m.baseline.carbonG, (energyWh("large", 1000, 1000) / 1000) * gPerKwh);
});
